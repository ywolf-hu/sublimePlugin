...
class RsyncSSH(threading.Thread):
    ...
    def sync_from_remote(self):
        ...
        local_path = list(self.settings.get('remotes').keys())[0]
        # cygwin_local_path = "/cygdrive/d/ubuntu_share/test"
        # cygwin_local_path = check_output(["cygpath", local_path]).strip()
        remote_path = self.settings.get('remotes').get(local_path)[0].get('remote_path')+"/"
        remote_host = self.settings.get('remotes').get(local_path)[0].get('remote_host')
        remote_user = self.settings.get('remotes').get(local_path)[0].get('remote_user')
        # rsync_command = "rsync -v -zar " + remote_user+"@"+remote_host+":" + remote_path + " " + cygwin_local_path
        rsync_command = "rsync -v -zar " + remote_user+"@"+remote_host+":" + remote_path + " " + local_path
        # Add excludes
        for exclude in set(self.settings.get('excludes')):
            rsync_command += (" --exclude="+exclude)
        print("rsync_command: ", rsync_command)
        start_time = datetime.now()
        os.system(rsync_command)
        end_time = datetime.now()
        print('rsync from remote take time: ', end_time-start_time)
        return
    ...
    def run(self):
        ...
        for folder_path_full in self.view.window().folders():
            ...
            print("folder_path_basename: ", folder_path_basename)
            ...
            for remote_key in self.settings.get("remotes").keys():
                ...
                if remote_key != ".":
                    ...
                    print('container_folder: ', container_folder)
                    ...
                    print('prefix: ', prefix)
                    print('remote_key.startswith(container_folder): ', remote_key.startswith(container_folder))
                ...
                print('path_being_saved: ', self.path_being_saved)
...
class Rsync(threading.Thread):
    ...
    def run(self):
        ...
        print('os.path.isfile(self.specific_path): ', os.path.isfile(self.specific_path))
        ...
        # if self.specific_path and os.path.isfile(self.specific_path) and self.cyg_specific_path.startswith(self.local_path+"/"):
            # source_path      = self.cyg_specific_path
            # destination_path = self.destination.get("remote_path") + self.cyg_specific_path.replace(self.local_path, "")
        ...
        elif self.specific_path and os.path.isdir(self.specific_path) and self.cyg_specific_path.startswith(self.local_path+"/"):
            source_path      = self.cyg_specific_path + "/"
            destination_path = self.destination.get("remote_path") + self.cyg_specific_path.replace(self.local_path, "")
        ...
        # linhu: remove checking whether rsync on server for reduce time
        ...
        #     print('rsync_path: ', self.rsync_path)
        ...
        try:
            start_time = datetime.now()
            print('rsync start time: ', start_time)
            print('rsync_command: ', rsync_command)
            # output = check_output(rsync_command, stderr=subprocess.STDOUT)
            final_rsync_command=""
            for l_rsync_command in rsync_command:
                final_rsync_command += " "
                final_rsync_command += l_rsync_command
            print('final_rsync_command: ', final_rsync_command)
            output = os.system(final_rsync_command)
            if not output:
                status_bar_message = 'Sync successfully'
            else:
                status_bar_message = 'Sync failure'
            end_time = datetime.now()
            delta_time = end_time - start_time
            status_bar_message += (' ' + str(delta_time))
            self.view.set_status("00000_rsync_ssh_status", status_bar_message)
            print('rsync return value: ', output)
            print('rsync take time: ', delta_time)
